...
function Pf_diffusion_2D_gpu(;do_check = true, testing)
    ...
    while err_Pf >= ϵtol && iter <= maxiter
        ...
        # Flux computation and pressure update
        CUDA.@sync @cuda blocks=blocks threads=threads compute_flux_gpu!(Pf, qDx, qDy, k_ηf_dx, k_ηf_dy, _1_θ_dτ)
        CUDA.@sync @cuda blocks=blocks threads=threads update_Pf_gpu!(Pf, qDx, qDy, _dx, _dy, _β_dτ) 
        if do_check && (iter % ncheck == 0)
            r_Pf  .= diff(qDx, dims=1) ./ dx .+ diff(qDy, dims=2) ./ dy
            err_Pf = maximum(abs.(r_Pf))
            @printf("  iter/nx=%.1f, err_Pf=%1.3e\n", iter / nx, err_Pf)
...
Pf_cpu = Pf_diffusion_2D_cpu(do_check = false, testing = testing) #using 4 threads for CPU computations.
Pf_gpu = Pf_diffusion_2D_gpu(do_check = false, testing = testing)
