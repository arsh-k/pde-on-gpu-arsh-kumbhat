...
@views function update_halo!(A, neighbors_x, comm)
    ...
    if neighbors_x[1] != MPI.PROC_NULL #PROC_NULL - Returns null if we are near the physical boundary
        sendbuf = A[2]
        recvbuf = zeros(1)
...
@views function diffusion_1D_mpi(; do_save=false)
    ...
    MPI.Dims_create!(nprocs, dims) #Populates the dims array according to the number of processes.
    comm_cart = MPI.Cart_create(comm, dims, [0], 1)
    me     = MPI.Comm_rank(comm_cart) #This is analogous to process ID.
    coords = MPI.Cart_coords(comm_cart) #This provides us with the coordinates of the domain.
    neighbors_x = MPI.Cart_shift(comm_cart, 0, 1) #This indicates the neighbours of a local domain. 
    ...
    qx    = zeros(nx-1)
    # Initial condition
    x0    = coords[1] * (nx - 2) * dx #This is the first global coordinate for each process.
    xc    = [x0 + ix * dx - dx / 2 - 0.5 * lx for ix = 1:nx] #This is the local chunk of global coordinates for each process.
    C     = exp.(.-xc .^ 2)
    t_tic = 0.0
    # Time loop
    for it = 1:nt
    ...
    (me == 0) && @printf("Time = %1.4e s, T_eff = %1.2f GB/s \n", t_toc, round((2 / 1e9 * nx * sizeof(lx)) / (t_toc / (nt - 10)), sigdigits=2))
    if do_save file = matopen("../data/1D-diffusion/mpi1D_out_C_$(me).mat", "w"); write(file, "C", Array(C)); close(file) end
    MPI.Finalize()
    return
end
