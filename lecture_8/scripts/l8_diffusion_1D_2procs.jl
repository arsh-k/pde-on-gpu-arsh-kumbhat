...
@views function diffusion_1D_2procs(; do_visu=false)
    ...
    savefig("./docs/diffusion_1D_2procs.png")
    return
end

# If we do not update the boundaries, the code produces an abrupt plot indicating no connection 
# between the left part of the domain and the right part.
diffusion_1D_2procs(; do_visu=true)
# Plotting with the left and right hand halo updates (i.e., using fake parallelization), the same
# diffusion plot is obtained as without a split in the domain.