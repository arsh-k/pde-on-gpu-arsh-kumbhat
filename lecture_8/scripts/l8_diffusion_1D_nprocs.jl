...
@views function diffusion_1D_nprocs(; do_visu=true)
    ...
    savefig("../docs/diffusion_1D_nprocs.png")
    return
end

# Without updating the local boundaries we can clearly observe that there is no connection in between the segregated domain processes
# and the final concentration curve obtained is not smooth. (i.e. there is a discontinuity in the solution)
diffusion_1D_nprocs(; do_visu=true)
# After updating the local boundaries, we obtain a solution that matches with the solution obtained by solving for the complete domain.
# There exists no discontinuity within the solution.
