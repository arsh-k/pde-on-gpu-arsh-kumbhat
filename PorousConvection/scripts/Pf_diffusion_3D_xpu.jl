...
function Pf_diffusion_3D(; nx=511, ny=511, do_check=false)
    # physics
    lx, ly, lz  = 20.0, 20.0, 20.0
    k_ηf    = 1.0
    # numerics
    nx, ny, nz   = 8*32, 8*32, 8*32
    ϵtol    = 1e-8
    ...
    A_eff = (4 * 2) / 1e9 * nx * ny * sizeof(Float64)  # Effective main memory access per iteration [GB]
