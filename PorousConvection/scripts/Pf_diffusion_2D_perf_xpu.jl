...
function Pf_diffusion_perf_2D(; nx=511, ny=511, do_check=false)
    # physics
    lx, ly  = 20.0, 20.0
    k_ηf    = 1.0
    # numerics
    nx,ny   = 16*32, 16*32 #Grid points chosen as a multiple of 32 (kernel launch params are multiples of 32)
    ϵtol    = 1e-8
    ...
    # visu
    if do_check
        ENV["GKSwstype"] = "nul"; if (isdir("viz_out") == false) mkdir("viz_out") end; loadpath = "viz_out/"
        anim = Animation(loadpath, String[]); println("Animation directory: $(anim.dir)"); iframe = 0
    end
    # iteration loop
